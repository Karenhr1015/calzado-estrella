            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                ...
                <div
                    ...
                    <div class="w-full md:w-1/2">
                        <form class="flex items-center gap-3" action="{{ route('sizes.index') }}" method="GET">
                            ...
                            <div class="relative w-full">
                                ...
                                <input type="text" id="simple-search" name="value"
                        ...
                        <tbody>
                            @foreach ($sizes as $size)
                                <tr class="border-b dark:border-gray-700">
                                    ...
                                    <td class="px-4 py-3">{{ $size->value }}</td>
                                    <td class="px-4 py-3">
                                        <x-status :type="$size->status"></x-status>
                                    </td>
                                    <td class="px-4 py-3 max-w-[12rem] truncate">{{ $size->created_at }}</td>
                                    <td class="flex gap-5">
                                        ...
                                        <form action="{{ route('sizes.destroy', $size->id) }}" method="POST">
                                            ...
                                            <a :href="route('sizes.destroy', $size - > id)"
                                                onclick="event.preventDefault(); this.closest('form').submit()">
                                                <x-secondary-button>
                                                    <div class="flex gap-2 items-center">
                                                        <x-status-button :status="$size->status"></x-status-button>
                                                        {{ $size->status ? __('Inactivar') : __('Activar') }}
                                                    </div>
                                                </x-secondary-button>
                                            </a>
                                        </form>
