            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                ...
                <div
                    ...
                    <div class="w-full md:w-1/2">
                        <form class="flex items-center gap-3" action="{{ route('seasons.index') }}" method="GET">
                        ...
                        <tbody>
                            @foreach ($seasons as $season)
                                <tr class="border-b dark:border-gray-700">
                                    ...
                                    <td class="px-4 py-3">
                                        <x-status :type="$season->status"></x-status>
                                    </td>
                                    <td class="px-4 py-3 max-w-[12rem] truncate">{{ $season->created_at }}</td>
                                    <td class="flex gap-5">
                                        ...
                                        <form action="{{ route('seasons.destroy', $season->id) }}" method="POST">
                                            ...
                                            <a :href="route('seasons.destroy', $season - > id)"
                                                onclick="event.preventDefault(); this.closest('form').submit()">
                                                <x-secondary-button>
                                                    <div class="flex gap-2 items-center">
                                                        <x-status-button :status="$season->status"></x-status-button>
                                                        {{ $season->status ? __('Inactivar') : __('Activar') }}
                                                    </div>
                                                </x-secondary-button>
                                            </a>
                                        </form>
